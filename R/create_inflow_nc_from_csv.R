...
#' 
#' @param filename character; Point to csv file with headers ('date', flow_col, and any constituents).
#' 'date' needs to have format 'yyyy-mm-dd hh:mm:ss' and flow_col must be in m3/s
...
#' @param flow_col character; the name of the column with discharge values. default = "flow"
#' @author
#' Jorrit Mesman
#' @examples
#'  \dontrun{
...
create_inflow_nc_from_csv = function(filename, lat, lon, file_out,
                                     ...
                                     plot = FALSE, flow_col = "flow"){
  # Read file
  df = fread(filename)
  
  # Check validity of filename
  if(!("date" %in% names(df) & flow_col %in% names(df))){
  ...
  if(length(names(df) > 2)){
    col_names = names(df)[!(names(df) %in% c("date", flow_col, "temp", "salt"))]
    for(i in col_names){
      spl = strsplit(i, "_")[[1]]
      if(length(spl) == 1L){
  ...
  # Create netcdf
  times_nc = as.numeric(difftime(df_write$all_times, as.POSIXct(origin_getm, tz = "UTC"), units = "hours"))
  ...
  for(i in seq_along(vars)){
    the_unit = fifelse(vars[i] == flow_col, "m3/s", "-")

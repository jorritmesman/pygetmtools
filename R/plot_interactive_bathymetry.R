...
plot_interactive_bathymetry <- function(ncdf, coord_type = c("indices", "projected", "geographic")) {
  
  nc_file <- nc_open(ncdf)
  bath <- ncvar_get(nc_file, "bathymetry")
  ...
  bath_df <- melt(bath, varnames = coord_names, value.name = "depth")
